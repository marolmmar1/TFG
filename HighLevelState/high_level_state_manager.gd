	...
	for key in creature.memory.keys():
		if not typeof(key) == TYPE_STRING:
			...
			if not zone == creature.current_zone:
				...
				zones.append(parsed_zone) 
	var state = HighLevelState.new(creature.max_health,creature.max_food,creature.max_stamina,creature.health, creature.food, creature.stamina, current_zone,zones)
	return state	
